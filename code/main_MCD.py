...
#                  img_path = '../1-results/images/' + model_name)
    else:
        count = 0
...
def main():
    ...
    for i in range(sub_num):
        print('Test ', i)
        acc_s_MCD[i], acc_t_MCD[i] = classify(leave_one_num = i)
...
if __name__ == '__main__':
    ...
    acc_MCD_uci = main()
    np.savetxt("../1-results/acc_MCD_all_sensor_uci.csv",
                  acc_MCD_uci, delimiter=",")

    args.is_source_only = True

    acc_MCD_uci = main()
    np.savetxt("../1-results/acc_MCD_all_sensor_uci_souce_only.csv",
                  acc_MCD_uci, delimiter=",")
    ...
    acc_MCD_nw = main()
    np.savetxt("../1-results/acc_MCD_all_sensor_nw.csv",
              acc_MCD_nw, delimiter=",")

    args.is_source_only = True

    acc_MCD_nw = main()
    np.savetxt("../1-results/acc_MCD_all_sensor_nw_souce_only.csv",
              acc_MCD_nw, delimiter=",")
    ...
    for i in range(6):
        args.sensor_num = i + 1
        acc_MCD_nw[2*i:2*(i+1), :] = main()
    np.savetxt("../1-results/acc_MCD_compare_sensor_nw.csv",
              acc_MCD_nw, delimiter=",")
    ...
    for i in range(5):
        args.sensor_num = i + 1
        acc_MCD_uci[2*i:2*(i+1), :] = main()
    np.savetxt("../1-results/acc_MCD_compare_sensor_uci.csv",
              acc_MCD_uci, delimiter=",")
